...
public class Importer {
  public static Integer importLoadbalancer(String filename, String newLoadBalancerName,
      String newPayType) throws IOException {
    Integer errorCode = 0;
    ...
    CreateLoadBalancerResponse response =
        SLBHelperAPI.createLoadBalancer(loadBalancerName, loadBalancerSpec, regionId, masterZoneId,
            slaveZoneId, addressType, vpcId, vSwitchId, internetChargeType, bandwidth);

    newLoadBalancerId = response.getLoadBalancerId();
    // 添加后端服务器

    SLBHelperAPI.addBackendServers(newLoadBalancerId, regionId,
        loadBalancerObject.getBackendServers());

    // 创建虚拟服务器组
    Map<String, String> gMap = new HashMap<>();
    List<VServerGroup> vsgs = loadBalancerObject.getvServerGroups();
    if (vsgs != null) {
      vsgs.forEach(vsg -> {
    ...
    // 创建主备服务器组
    List<MasterSlaveServerGroup> msgs = loadBalancerObject.getMasterSlaveGroups();
    if (msgs != null) {
      msgs.forEach(msg -> {
        ...
        switch (listen.getProtocol()) {
          ...
          case "udp":
            SLBHelperAPI.createLoadBalancerUDPListener(loadBalancerObject.getRegionIdAlias(),
                ...
                listen.getBackendServerPort(), listen.getRealServerType(),
                gMap.get(listen.getRealServerId()), listen.getBandwidth(), listen.getScheduler(),
                listen.getHealthyThreshold(), listen.getUnhealthyThreshold(),
                ...
                ((UDPListener) listen).getHealthCheckReq());
            break;
          ...
          case "http":
            SLBHelperAPI.createLoadBalancerHTTPListener(loadBalancerObject.getRegionIdAlias(),
                ...
                listen.getHealthCheckInterval(), listen.getHealthCheckDomain(),
                listen.getHealthCheckURI(), listen.getHealthCheckHttpCode(),
                listen.getHealthCheckConnectPort(), ((HTTPListener) listen).getCookie(),
                ((HTTPListener) listen).getCookieTimeout(),
                ((HTTPListener) listen).getStickySession(),
                ((HTTPListener) listen).getStickySessionType(), ((HTTPListener) listen).getGzip(),
                ((HTTPListener) listen).getxForwardedFor_SLBIP(),
                ((HTTPListener) listen).getxForwardedFor_SLBID(),
                ((HTTPListener) listen).getxForwardedFor_proto(),
